...
const DarkStatsSection = ({ heading = "The Result", introText, stats }: DarkStatsSectionProps) => {
  return (
    <section className="bg-[hsl(0_0%_7%)] text-background -mx-6 px-6 py-20 my-20 rounded-none">
      <div className="max-w-4xl mx-auto">
        <div className="grid md:grid-cols-[35%_65%] gap-16">
          {/* Left column */}
          <div>
            <h2 className="text-[28px] font-bold text-background mb-4">{heading}</h2>
          ...
          <div>
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`flex items-baseline gap-6 py-7 border-b border-[hsl(0_0%_16%)] ${index === 0 ? 'border-t' : ''}`}
              >
                <div className="text-[60px] font-extralight text-background leading-none min-w-[140px]">
                  {stat.number}
                </div>
